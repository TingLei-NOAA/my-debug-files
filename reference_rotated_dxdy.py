"""
Generate a reference dx/dy field for the configured rotated_lonlat StructuredColumns grid.

Grid setup (inclusive endpoints):
...
Outputs:
  dr-figures/reference_dx_km.png
  dr-figures/reference_dy_km.png
  dr-figures/reference_dx_over_dy.png
  dr-figures/reference_lon.png
  dr-figures/reference_lat.png
"""
...
def compute_dx_dy(lon: np.ndarray, lat: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    # Pad boundaries with nearest interior value to keep shape
...
def plot_field(field: np.ndarray, lon: np.ndarray, lat: np.ndarray, title: str, outfile: Path, cmap: str, units: str):
...
def main():
    parser = argparse.ArgumentParser(description="Reference dx/dy for rotated_lonlat StructuredColumns grid")
    ...
    plot_field(dx / 1000.0, lon, lat, "Reference dx (km)", args.output_dir / "reference_dx_km.png", cmap="magma", units="km")
    plot_field(dy / 1000.0, lon, lat, "Reference dy (km)", args.output_dir / "reference_dy_km.png", cmap="magma", units="km")
    plot_field(ratio, lon, lat, "Reference dx/dy", args.output_dir / "reference_dx_over_dy.png", cmap="coolwarm", units="ratio")
    plot_field(lon, lon, lat, "Reference lon", args.output_dir / "reference_lon.png", cmap="coolwarm", units="deg")
    plot_field(lat, lon, lat, "Reference lat", args.output_dir / "reference_lat.png", cmap="coolwarm", units="deg")
